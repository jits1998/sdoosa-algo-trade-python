...
class NiftySell1400PR20(BaseStrategy):
  __instance = {}

  @staticmethod
  def getInstance(short_code): # singleton class
    if NiftySell1400PR20.__instance.get(short_code, None) == None:
      NiftySell1400PR20()
    return NiftySell1400PR20.__instance[short_code]

  def __init__(self, short_code, multiple = 0):
    if NiftySell1400PR20.__instance.get(short_code, None) != None:
      raise Exception("This class is a singleton!")
    else:
      NiftySell1400PR20.__instance[short_code] = self
    # Call Base class constructor
    super().__init__("NiftySell1400PR20", short_code, multiple)
  ...
  def process(self):
    ...
    if takeCETrade:
      ceStrike, cePremium = self.getStrikeWithMaximumPremium("CE", 20, roundToNearestStrike=50)
    ...
    if takePETrade:
      peStrike, pePremium = self.getStrikeWithMaximumPremium("PE", 20, roundToNearestStrike=50)
