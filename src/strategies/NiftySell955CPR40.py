...
class NiftySell955CPR40(BaseStrategy):
  __instance = {}

  @staticmethod
  def getInstance(short_code): # singleton class
    if NiftySell955CPR40.__instance.get(short_code, None) == None:
      NiftySell955CPR40()
    return NiftySell955CPR40.__instance[short_code]

  def __init__(self, short_code, multiple = 0):
    if NiftySell955CPR40.__instance.get(short_code, None) != None:
      raise Exception("This class is a singleton!")
    else:
      NiftySell955CPR40.__instance[short_code] = self
    # Call Base class constructor
    super().__init__("NiftySell955CPR40", short_code, multiple)
    # Initialize all the properties specific to this strategy
    self.productType = ProductType.MIS
    self.slPercentage = 0
    self.targetPercentage = 0
    self.startTimestamp = Utils.getTimeOfToDay(9, 55, 0) # When to start the strategy. Default is Market start time
    self.stopTimestamp = Utils.getTimeOfToDay(13, 59, 0) # This is not square off timestamp. This is the timestamp after which no new trades will be placed under this strategy but existing trades continue to be active.
    self.squareOffTimestamp = Utils.getTimeOfToDay(13, 59, 0) # Square off time
  ...
  def process(self):
    ...
    if takeCETrade:
      ceStrike, cePremium = self.getStrikeWithMinimumPremium("CE", 40, roundToNearestStrike=50)
    ...
    if takePETrade:
      peStrike, pePremium = self.getStrikeWithMinimumPremium("PE", 40, roundToNearestStrike=50)
