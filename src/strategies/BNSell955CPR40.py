...
class BNSell955CPR40(BaseStrategy):
  __instance = {}

  @staticmethod
  def getInstance(short_code): # singleton class
    if BNSell955CPR40.__instance.get(short_code, None) == None:
      BNSell955CPR40()
    return BNSell955CPR40.__instance[short_code]

  def __init__(self, short_code, multiple = 0):
    if BNSell955CPR40.__instance.get(short_code, None) != None:
      raise Exception("This class is a singleton!")
    else:
      BNSell955CPR40.__instance[short_code] = self
    # Call Base class constructor
    super().__init__("BNSell955CPR40", short_code, multiple)
  ...
  def process(self):
    ...
    if takeCETrade:
      ceStrike, cePremium = self.getStrikeWithMinimumPremium("CE", 40)
    ...
    if takePETrade:
      peStrike, pePremium = self.getStrikeWithMinimumPremium("PE", 40)
